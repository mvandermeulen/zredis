...
static int redis_connect(redisContext **rc, const char* password, const char *host, int port, int db_index, const char *address);
...
zrtie_cmd(int flags, char *address, char *pass, char *pfile, char *pmname, char *lazy)
{
    ...
    if (!lazy || (flags & DB_FLAG_NOCONNECT) == 0) {
        if (!redis_connect(&rc, pass, host, port, db_index, address)) {
            return 1;
        } else {
...
/* FUNCTION: connect {{{ */
static int
redis_connect(redisContext **rc, const char* password, const char *host, int port, int db_index, const char *address)
{
    redisReply *reply = NULL;
...
reconnect(redisContext **rc, int *fdesc, const char *hostspec_in, const char *password)
{
    ...
    *rc = NULL;
    ...
    if(!redis_connect(rc, password, host, port, db_index, hostspec_in)) {
        *rc = NULL;
